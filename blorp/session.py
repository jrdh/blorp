...
def save_session(session):
    ...
    yield from pool.async_pool.set(session_id_template.format(websocket_id), json.dumps(session), expire=default_ttl)
...
def delete_session(websocket_id):
    yield from pool.async_pool.delete([session_id_template.format(websocket_id)])


@asyncio.coroutine
def touch_session(websocket_id):
    yield from pool.async_pool.expire(session_id_template.format(websocket_id), default_ttl)


@asyncio.coroutine
def get_session(websocket_id, create=True):
    session = yield from pool.async_pool.get(session_id_template.format(websocket_id))
    if session:
